...
\*******************************************************************/
	...
	if (isset($_GET['show']) && is_numeric($_GET['show']) && $_GET['show'] > 0)
		...
		$query = "SELECT *, DATE_FORMAT(created, '".DATE_FORMAT."') AS signup_date FROM exchangerix_users $where ORDER BY $rrorder $rorder LIMIT $from, $results_per_page";
...
?>
       ...
       <div id="addnew">
			<i class="fa fa-user" style="color: #f39425"></i> <a href="users.php?only_verifications=1" style="margin-right: 10px">Verification Requests <?php if (GetVerificationRequestsTotal() > 0) { ?><sup class="badge" style="background: #f39425"><?php echo GetVerificationRequestsTotal(); ?></sup><?php } ?></a>
			<?php if ($total > 0) { ?>
       ...
       <h2><i class="fa fa-users" aria-hidden="true"></i> <?php echo @$title2; ?> Users <?php echo @$totitle; ?> <?php if ($total > 0) { ?><sup class="badge" style="background: #73b9d1"><?php echo number_format($total); ?></sup><?php } ?></h2>
		...
		<div class="col-md-5 text-center" style="white-space: nowrap">
				<div class="admin_filter">
					<input type="text" name="filter" value="<?php echo $filter; ?>" placeholder="Enter keyword" class="form-control" size="15" required="required" />
        ...
        <?php if ($total > 0) { ?>
			...
			<tr bgcolor="#DCEAFB" align="center">
				...
				<th width="8%">Referrals<!--Country--></th>
				<!--<th width="8%">IP</th>
			 ...
			 <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
					...
					<td align="left" valign="middle" style="padding-left: 10px">
						...
						<?php if ($row['phone'] != "") { ?><br><span style="color: #777"><i class="fa fa-phone-square"></i> <?php echo $row['phone']; ?></span><?php } ?>
					</td>
					...
					<td align="left" valign="middle" style="padding-left: 10px"><?php echo ($row['status'] == "inactive") ? "<span class='label label-default'>inactive</span>" : "<span class='label label-success'>active</span>"; ?></td>
					<td align="center" valign="middle" nowrap="nowrap">
