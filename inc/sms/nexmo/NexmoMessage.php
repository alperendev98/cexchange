...
class NexmoMessage {
	...
	function sendVoice($to , $text){
		$text = $this->nexmo_conv($text);
		$url = "https://rest.nexmo.com/tts/json?api_key=$this->nx_key&api_secret=$this->nx_secret&to=$to&text=$text&lg=zh-cn";

		return $this->nexmoParse(file_get_contents($url));
	...
	public function nexmo_conv( $string ) {
		$conv = new converter;
